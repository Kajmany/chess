...
type Info struct {
	Depth             int
	Seldepth          int
	PVs               []PV //My patch for MPV support
	Multipv           int
	Time              time.Duration
	Nodes             int
	Score             Score
...
// My patch for MPV support
type PV struct {
	Moves []string //Sue me. It's going into JSON anyway, hah!
	Score int
	Rank  int
}
...
func (info *Info) UnmarshalText(text []byte) error {
	...
	for i := 1; i < len(parts); i++ {
		...
		//My patch: The PV case and conditional guard below was removed because the logic is handled in uci.CmdGo now
		ref = ""
	}
	return nil
}
